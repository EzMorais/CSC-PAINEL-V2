...
// mas sem o módulo Painel liberado no Portal não pode ler nada do Painel. Antes desta
...
func TestSessao_BloqueiaSemModuloLiberado(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/painel", nil)
	req.AddCookie(cookie)

	if _, ok := h.sessao(rec, req); ok {
		t.Fatal("esperava bloqueio: sessão não tem ModuloPainel liberado")
	}
...
func TestSessao_LiberaComModulo(t *testing.T) {
	...
	h := &Handlers{Sessoes: s}
	cookie := cookieDe(t, s, identidade.Sessao{
		ID: "u1", Cargo: identidade.CargoOperacional, Modulos: []identidade.Modulo{identidade.ModuloPainel},
	})
	...
	if _, ok := h.sessao(rec, req); !ok {
		t.Fatalf("esperava liberar: sessão tem ModuloPainel; status=%d", rec.Code)
	}
}
