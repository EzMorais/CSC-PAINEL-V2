...
// mas sem o módulo RH liberado no Portal não pode ler nada do módulo. Antes desta correção,
// qualquer sessão válida passava — ver ERP_BRIEFING_MELHORIA.md item 1.
func TestSessao_BloqueiaSemModuloLiberado(t *testing.T) {
	...
	if _, ok := h.sessao(rec, req); ok {
		t.Fatal("esperava bloqueio: sessão não tem ModuloRH liberado")
	}
...
func TestSessao_LiberaComModulo(t *testing.T) {
	...
	if _, ok := h.sessao(rec, req); !ok {
		t.Fatalf("esperava liberar: sessão tem ModuloRH; status=%d", rec.Code)
	}
}
