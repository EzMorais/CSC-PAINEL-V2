...
// mas sem o módulo Alojamentos liberado no Portal não pode ler nada do módulo. Antes desta
...
func TestSessao_BloqueiaSemModuloLiberado(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/alojamentos", nil)
	req.AddCookie(cookie)

	if _, ok := h.sessao(rec, req); ok {
		t.Fatal("esperava bloqueio: sessão não tem ModuloAlojamentos liberado")
	}
...
func TestSessao_LiberaComModulo(t *testing.T) {
	...
	h := &Handlers{Sessoes: s}
	cookie := cookieDe(t, s, identidade.Sessao{
		ID: "u1", Nome: "Ana", Cargo: identidade.CargoOperacional, Modulos: []identidade.Modulo{identidade.ModuloAlojamentos},
	})
	...
	nome, ok := h.sessao(rec, req)
	if !ok {
		t.Fatalf("esperava liberar: sessão tem ModuloAlojamentos; status=%d", rec.Code)
	}
	if nome != "Ana" {
		t.Fatalf("esperava nome da sessão devolvido, veio %q", nome)
	}
}
