...
test.describe('Documentos — Go', () => {
  ...
  test('lista os 3 documentos do seed, empresa e pessoal juntos', async ({ page }) => {
    ...
    // getByRole('link', ...) porque "PGR" aparece duas vezes na linha (título e categoria).
    await expect(page.getByTestId('lista-documentos').getByRole('link', { name: 'PGR' })).toBeVisible()
    await expect(page.getByTestId('lista-documentos')).toContainText('PCMSO')
  })
  ...
  test('reenviar um documento com o mesmo título/categoria/obra cria versão 2, não sobrescreve', async ({
    ...
    await page.goto('/rh/documentos')
    await page.getByTestId('lista-documentos').getByRole('link', { name: 'PGR' }).click()
