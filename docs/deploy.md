...
O repositório tem 8 aplicações. Seis delas dividem um **login único**: entrar no Portal
deixa a pessoa já autenticada no Painel de Locação, RH, Almoxarifado, Programação e
Alojamentos. Isso funciona por causa de um cookie de sessão que o Portal emite e os outros
conferem — e **cookie sem domínio explícito vale para o mesmo host, em qualquer porta, mas
não atravessa hosts diferentes**. Por isso, ao contrário do que se poderia imaginar, esses
seis sistemas **não** ganham cada um um subdomínio bonito (`rh.suaempresa.com`,
`estoque.suaempresa.com`...) — eles continuam no mesmo domínio, diferenciados por porta,
exatamente como quando você roda tudo em `localhost` no seu computador. Trocar isso por
subdomínios separados quebraria o "loga uma vez, circula em todos".
...
Portal, Painel de Locação, RH, Almoxarifado, Programação, Alojamentos e WhatsApp precisam
do **mesmo** `AUTH_SECRET`. Gere um valor:

```bash
openssl rand -base64 48
```

Cole o resultado no lugar de `COLOQUE_O_SEGREDO_COMPARTILHADO_DOS_6_SISTEMAS_AQUI` nos 7
arquivos:
...
> Portal ser aceito nos outros. Um valor diferente em qualquer um deles não dá erro visível
> — a pessoa simplesmente é jogada de volta pro login ao entrar naquele sistema, o que é
> confuso de diagnosticar. Se isso acontecer depois do deploy, confira primeiro se os 6
> `AUTH_SECRET` (Portal a Alojamentos) são byte-a-byte iguais.
...
A primeira vez demora — está compilando as 8 aplicações. Acompanhe:
...
for v in portal_data painel_locacao_data rh_data estoque_data programacao_data alojamentos_data frota_dados; do
...
Os `AUTH_SECRET` não estão idênticos entre os 6 sistemas do grupo de login único. Confira
byte a byte (Passo 5.2) — é a causa em praticamente todos os casos.
...
Não é só trocar no `docker-compose.yml` — o nginx, os `NEXT_PUBLIC_URL_*` de todos os 6
apps do grupo de login único e o firewall também mudam. Evite, a menos que seja realmente
necessário.
